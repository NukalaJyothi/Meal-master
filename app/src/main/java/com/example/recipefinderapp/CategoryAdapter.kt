...
import com.example.recipefinderapp.databinding.CategoryRvBinding
...
class CategoryAdapter (var dataList:ArrayList<Recipe>, var context: Context):RecyclerView.Adapter<CategoryAdapter.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        var temp = dataList.get(position).ing.split("/n").dropLastWhile { it.isEmpty() }.toTypedArray()
        holder.binding.time.text = temp[0]
    }
}

//The main bench of high court in Rajasthan is at Jodhpur and another bench is in the capital city Jaipur It is very important that we start doing 5 projects this week because we have to show these projects on monday because on monday we have lab